class OrdersController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @order.update(order_params)
        @order.requisition.update(status: 'Shipped')
        RestClient::Resource.new(APP_CONFIG[:insight_url], :verify_ssl => false, :user => APP_CONFIG[:insight_username], :password => APP_CONFIG[:insight_password]).post({
            ...
            "correlationValue" => @order.requisition.id,
            "eventTime" => Time.now.strftime('%Y-%m-%dT%H:%M:%S'),
            "milestoneId" => "SuppliesSent"
  ...
  private
    ...
    def order_params
      params.require(:order).permit(:sent, :comments, :status, :requisition_id)
    end
end
